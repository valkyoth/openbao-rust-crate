...
impl Transit<'_> {
    ...
    ///
    /// This default endpoint wrapper never accepts raw private or symmetric
    /// key bytes and does not perform RSA-OAEP/AES-KWP wrapping. Fetch
    /// [`Transit::wrapping_key`], wrap private or symmetric key material
    /// externally through an HSM, OpenSSL, or reviewed crypto library, and pass
    /// only the base64 ciphertext blob in `request`.
    ...
    /// accepts only the already-wrapped BYOK ciphertext or public-key-only
    /// import material; raw private or symmetric key bytes remain outside this
    /// crate's default endpoint wrapper.
