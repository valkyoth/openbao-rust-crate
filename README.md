...
<div align="center">
  ...
  ·
  <a href="docs/CURRENT_STATUS.md">Current Status</a>
  ·
...
documented in [API Coverage](docs/OPENBAO_API_COVERAGE.md), with exact
operation/profile classifications in the
[OpenBao Version Support Matrix](docs/OPENBAO_VERSION_SUPPORT_MATRIX.md).
Feature history and release details live in [CHANGELOG.md](CHANGELOG.md) and
the [GitHub releases](https://github.com/valkyoth/openbao-rust-crate/releases).
...
[signed repository source](https://github.com/valkyoth/openbao-rust-crate/tree/v2.1.3/release-notes)
and
...
| `1.1.0` through `1.1.2` | Release-tested with OpenBao `2.5.5`. | Historical result for that SDK line; upgrade to `2.1.3` for the newest reviewed OpenBao profile. |
...
| `2.1.2` through `2.1.3` | Exact contract profiles and representative live core flows for all 23 published releases from `2.0.0` through `2.6.1`; six additional 2.6-only flows run on exact `2.6.0` and `2.6.1`. | OpenBao `2.6.1` is the newest reviewed server. Older profiles remain compatibility targets, not security endorsements. |

The immutable source and OCI artifact inventory for those historical releases
is committed under
[`compat/`](https://github.com/valkyoth/openbao-rust-crate/tree/v2.1.3/compat),
...
[`tests/fixtures/openbao_response_profiles.json`](https://github.com/valkyoth/openbao-rust-crate/blob/v2.1.3/tests/fixtures/openbao_response_profiles.json)
...
Use the current `2.x` crate even when the server is older. Pin the server's real,
...
Use `automatic_strict()` when the application may connect to any one of the 22
...
The `2.x` typed dispatcher selects one immutable operation variant before
...
Every client limits authentication tokens to 16 KiB before allocating an HTTP
header. Deployments with an intentionally larger OpenBao custom-token format
can use `OpenBaoConfig::max_auth_token_bytes` to raise that bound explicitly,
up to the 1 MiB hard ceiling. High-assurance deployments should retain the
default unless their token format and memory quotas have been reviewed.
...
| `memory-lock` | no | Stores each authenticated `Client`'s retained token in `sanitization::LockedSecretString`, requires an active OS lock plus OS-random corruption-detection canaries, enforces the configured token bound, and adds `try_with_locked_token` for direct mapped custody. Canaries detect accidental or adjacent corruption; they are not an attacker-resistant integrity boundary. Other request/response `SecretString`/`SecretVec` values and transport buffers are not automatically locked. Requires `memory-lock-acknowledged`. |
| `memory-lock-acknowledged` | no | Explicit acknowledgment that active token locking depends on audited OS limits and failure policy and does not cover ordinary response fields or HTTP/TLS/kernel/device copies. |
...
The repository also declares OpenSSL as a development dependency so its live
OpenBao test can generate valid RS256 JWTs. Development dependencies are not
compiled into a downstream application's normal dependency build. OpenSSL
enters production builds only when a caller explicitly enables a feature that
uses it, such as `transit-import`, or selects the acknowledged native-TLS
backend. The default Rustls feature set does not compile OpenSSL.
...
[docs/OPENBAO_2_5_ENDPOINT_MATRIX.md](https://github.com/valkyoth/openbao-rust-crate/blob/v2.1.3/docs/OPENBAO_2_5_ENDPOINT_MATRIX.md)
and
[`docs/openbao-2.5-contract-matrix.json`](https://github.com/valkyoth/openbao-rust-crate/blob/v2.1.3/docs/openbao-2.5-contract-matrix.json).
...
| Database credentials | Yes for built-ins | Connection lifecycle, dynamic/static roles and credentials, and root/static rotation helpers. Reviewed typed connection options cover PostgreSQL, the MySQL/MariaDB family, Cassandra, InfluxDB, and Valkey with secret-aware DSNs, passwords, TLS material, and role credential configuration. `DatabaseCredentialConfig` treats every value as `SecretString` because client-certificate roles can contain a CA private key. External plugin versions remain deployment-specific; unknown connection extension values also fail closed as `SecretString` and cannot shadow typed fields. |
...
| Admin bootstrap | Yes | Idempotent plan builder, read-only preview, KV v2/Transit/PKI/database/SSH mounts, Transit keys, ACL policies, KV v2 string values, PKI/database/SSH/AppRole roles, Identity entities/groups, explicit token issuance, and explicit AppRole SecretID issuance. Preview marks enabled identity-template delimiter overrides as unsafe drift; apply fails before replacing ACL, PKI, or SSH configuration so partial desired state cannot discard unmanaged security controls. |
...
```rust,no_run
use openbao::auth::jwt::{
    JwtCelClaimValidationAcknowledgement, JwtCelProgram, JwtCelRoleRequest,
};
use openbao::{Authenticated, Client, Result};

async fn configure_cel_role(client: &Client<Authenticated>) -> Result<()> {
    let program = JwtCelProgram::new(
        "claims.iss == 'issuer.example' && claims.sub == 'service' && \
         'openbao' in claims.aud ? pb.Auth{display_name: 'service'} : false",
    )?;
    let mut role = JwtCelRoleRequest::new(program).acknowledge_claim_validation(
        JwtCelClaimValidationAcknowledgement::all_authorization_claims_are_constrained_in_cel(),
    );
    role.bound_audiences = vec!["openbao".to_owned()];
    client.jwt_admin()?.write_cel_role("service", &role).await?;
    Ok(())
}
```
...
[`v2.1.3` signed source](https://github.com/valkyoth/openbao-rust-crate/tree/v2.1.3).
...
[Kani documentation](https://github.com/valkyoth/openbao-rust-crate/blob/v2.1.3/kani/README.md)
