...
| `memory-lock` | no | Exposes `sanitization` mapped-memory secret types through `openbao::sanitization`. It does **not** replace SDK-owned `SecretString`/`SecretVec` fields with locked or guarded allocations. Requires `memory-lock-acknowledged`; callers must explicitly use mapped types and audit OS limits and failure policy. |
| `memory-lock-acknowledged` | no | Explicit acknowledgment for applications that directly use the exposed mapped-memory types. It does not assert that SDK-owned or HTTP/TLS/kernel buffers are locked. |
