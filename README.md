...
| `memory-lock` | no | Stores each authenticated `Client`'s retained token in `sanitization::LockedSecretString`, fails closed if OS locking or integrity verification fails, and adds `try_with_locked_token` for direct mapped custody. Other request/response `SecretString`/`SecretVec` values and transport buffers are not automatically locked. Requires `memory-lock-acknowledged`. |
