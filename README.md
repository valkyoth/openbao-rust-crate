...
- PKI URL and CRL config, root/intermediate generation, intermediate signing
  ...
  list/read, issuer/key list/read/delete, ACME config/EAB, CRL rotate, and
  tidy helpers.
...
- `0.4.0`: remaining PKI import/update/revoke issuer and ACME protocol
  directory helpers.
...
| `pki` | yes | PKI authority, issuer/key metadata, role, issue/sign, revoke, cert read/list, ACME config/EAB, CRL config/rotate, and tidy helpers. |
...
| PKI | Partial | Authority generation/signing/install, URL/CRL config, roles, issue, sign, revoke, certificate list/read, issuer/key list/read/delete, ACME config/EAB, CRL rotate, and tidy are implemented. |
