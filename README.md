...
| Admin bootstrap | Yes | Idempotent plan builder, read-only preview, KV v2/Transit/PKI/database/SSH mounts, Transit keys, ACL policies, KV v2 string values, PKI/database/SSH/AppRole roles, Identity entities/groups, explicit token issuance, and explicit AppRole SecretID issuance. ACL, PKI, and SSH convergence treats enabled identity-template delimiter overrides as unsafe drift and verifies that ordinary writes clear them. |
