...
| PKI | Core CA, issuer/key, role, tidy, ACME config/EAB, issue/sign/revoke helpers. Matrix shows advanced issuer/root/public-read, config, revocation, CEL, and sign-verbatim gaps. | Implement named-issuer/root/public-read/config rows in `0.12.0`; implement revocation/CRL management, CEL, sign-verbatim, and cross-sign rows in `0.13.0`; classify OCSP as raw binary protocol support and keep full ACME account/order/authorization/challenge flows external. |
