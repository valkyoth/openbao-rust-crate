...
| System backend | Version-aware sys coverage includes ACL, lease, auth-mount, rotation, bounded monitor streaming, exact-length `raft-stream` restore helpers, and gated internal diagnostics. | Operator ceremonies stay behind `operator-ops` plus `operator-ops-acknowledged`; unstable internals additionally require `unstable-internal-ops-acknowledged`. Generated-token decoding is local because OpenBao documents no HTTP decode endpoint. |
