...
| Response wrapping | Sys wrapping lookup/wrap/unwrap/rewrap helpers exist, and `Client::wrapping` provides `WrappingContext` plus `WrappedResponse<T>` for ordinary typed JSON response wrapping with redacted wrapping tokens. `try_unwrap(&mut self)` preserves local ownership across cancellation and marks successful redemption consumed. | Per-engine wrapped method duplication is rejected; callers use the wrapping context, avoid automatic retry after outcome-unknown failures, and keep delivery/recipient policy outside the SDK. |
