...
| PKI | Core CA, issuer/key, role, tidy, ACME config/EAB, issue/sign/revoke helpers, default issuer/key config, named-issuer issue/sign, root rotate/replace, key generation, revoke-with-key, cluster/auto-tidy config, operator-gated sign-verbatim helpers, current-doc role/generation/CRL/tidy fields, and operator-gated default root deletion with explicit confirmation are typed. | `0.12.0` resolves the Tier 1 PKI multi-issuer and authority lifecycle scope. `DELETE /pki/root` remains resolved as `Pki::delete_root(PkiRootDeletion::confirm())`. Implement Tier 2 revocation/CRL management, CEL, named-issuer sign-intermediate/sign-self-issued, delta CRL rotation, and cross-sign rows in `0.13.0`; classify unauthenticated public CA/cert/CRL reads plus OCSP as external protocol/public-distribution endpoints and keep full ACME account/order/authorization/challenge flows external. |
